...
rsyncArgP :: Environment -> Query -> Parser RsyncArg
rsyncArgP e q =
  argument textRead $
       metavar "RSYNC_ARGUMENTS"
    <> help "Extra arguments to pass to ssh."
    <> action "file"
    <> completer (rsyncCompleter e q)
...
rsyncCompleter :: Environment -> Query -> Completer
rsyncCompleter env qTarget =
  mkCompleter $ \case
      ...
      -> do box <- getExecutablePath
            let
              envarg = case env of
                        SomeEnv x -> ["-e ", x]
                        DefaultEnv -> []
              args' = envarg <> ["ssh"] <> [ queryRender qTarget, "--", "ls", "-aF1dL", T.pack arg <> "*"]
              box'  = readProcessWithExitCode box (fmap T.unpack args') []
    ...
    _
      -> return []
  where
    prefixedlines (_code, lsoutput, _stderr) = fmap (':':) $ lines lsoutput
